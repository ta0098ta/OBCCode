...
INT8U OSSemPost_i (OS_EVENT *pevent)
{
    //OS_ENTER_CRITICAL();
    if (pevent->OSEventType != OS_EVENT_TYPE_SEM) {        /* Validate event block type                */
       // OS_EXIT_CRITICAL();
    ...
    if (pevent->OSEventGrp) {                              /* See if any task waiting for semaphore    */
        (void)OS_EventTaskRdy(pevent, (void *)0, OS_STAT_SEM, OS_STAT_PEND_OK);    /* Ready highest prio task waiting on event */
       // OS_EXIT_CRITICAL();
        OSSched_i();                                    /* Find highest priority task ready to run       */
        return (OS_NO_ERR);
    } else {
        if (pevent->OSEventCnt < 65535u) {             /* Make sure semaphore will not overflow         */
            pevent->OSEventCnt++;                     /* Increment semaphore count to register event   */
