...
static uchar *rxbuf = (uchar *)&((immap_t *)CFG_IMMR)->im_dprambase[CONFIG_SYS_SPI_INIT_OFFSET];
...
void spi_init_r()/*2013*/
{
        ...
        spi  = (spi_t *)&immr->im_dprambase[PROFF_SPI_BASE];
 ...
 *  Function:    spi_write
 *
 *  写函数可以这么理解，将 两个数组都设置为 即将发送的 数组。 
 *  其中有一个问题，就是  整个spi.c 文件中，有几个红定义是 ppcboot里的，不知在ucos 系统中用起来会不会出问题。
 **************************************************************************** */
...
int  spi_write_test (uchar testdata)
{
        int i;
       // INT8U err;
       // mutex = OSMutexCreate(MUTEX_PIRO,&err);
       // OSMutexPend (MUTEX_PIRO, 0 , &err);
...
/*
        ...
        */
        ...
        return rxbuf[0];
        //OSMutexPost (MUTEX_PIRO);
}

/****************************************************************************
 *  Function:    spi_read
 *
 *读函数  第一个 参数 是 之前的指令数组， 第三个参数 是 用来接收 返回数据的 数组。
 **************************************************************************** */
